<?php
/**
 * config.php
 * 扩展应用 example
 * date 2018-04-16 00:08:34
 * @copyright ECDO. All Rights Reserved.
 */
 
namespace Gtd\Extension\User\Http\Controllers\Site\User;

use Illuminate\Http\Request;
use Gtd\Extension\User\Http\Controllers\Site\SiteUserController;
use Illuminate\Support\Carbon;
use Illuminate\Filesystem\Filesystem;
use Illuminate\Support\Str;

use Gtd\Extension\User\Models\Invite;
use Gtd\Extension\User\Models\Product;

class InviteController extends SiteUserController
{   
    public function __construct(){
        
        $this->objectModel = new Invite;
        
        $this->setData('sidebar_active','invite');
        
        parent::__construct();
    }
    
    public function index(Request $request,$tab_active='all'){
        
        $this->title('Invite');
        
        $page_no = 0;
        $page_size=20;
        if($request->get('page')){
            $page_no = $request->get('page');
        }
        $request_data = $request->all();
        $this->_filter($request_data,$page_size,$page_no,$data);
        
        $this->setData('data_list',$data);
        
        $this->setData('tab_active',$tab_active);
        
        return $this->display('center.invite.index');
    }

    public function add(Request $request)
    {
        if(!$this->user->badge_certificate)
        {
            return $this->responseAjax('fail','认证用户可发起邀请');
        }
        $in_count = Invite::where('user_id',$this->user->id)->count();
        if($in_count >= 20){
            return $this->responseAjax('fail','已达到邀请最大数量');
        }

        $in_count = Invite::where('user_id',$this->user->id)->where('invite_user_id',0)->count();
        if($in_count >= 5)
        {
            return $this->responseAjax('fail','已达到邀请最大数量');
        }

        $inviteObj = new Invite;
        $inviteObj->user_id = $this->user->id;
        $inviteObj->invite_code = $this->makeCode();

        $inviteObj->save();
        
        return $this->responseAjax('success','已生成邀请码','self.refresh');
    }

    public function reset(Request $request,$invite_code){
        
        $this->title('Invite');
        $this->setData('modal_title','Invite');
        
        
        $invite = Invite::where('invite_code',$invite_code)->where('user_id',$this->user->id)->first();
        
        if($invite){
            if($invite->invite_user_id){
                return $this->responseAjax('fail','已使用不可重置');
            }

            Invite::where('invite_code',$invite_code)->where('user_id',$this->user->id)->update([
                'invite_code' => $this->makeCode(),
            ]);

            return $this->responseAjax('success','重置成功','self.refresh');
        }
        return $this->responseAjax('fail','数据不存在');
    }
    
    
    protected function makeCode()
    {
        do{
            $i = strtolower(Str::random(8));
            $row = Invite::where(['invite_code'=>$i])->first();
        }while($row);
        return $i;
    }

    public function _filter($filter=[],$page_size=20,$page_no=0,&$data){
        
        $objectModel = $this->objectModel;
        
        if($filter && array_key_exists('filter',$filter) && $filter['filter']=='true'){
            
            $comma = '';
            
            if(array_key_exists('page',$filter)){
                unset($filter['page']);
            }
            if(array_key_exists('filter',$filter)){
                unset($filter['filter']);
            }
            
            $sModel = $objectModel->where([]);
            
            foreach($filter as $k=>$v){
            
                //多选筛选
                if(is_string($v) && strpos($v,',')!=false){
                    $v = explode(',',$v);
                }
            
                if($k=='domain'){
                    $sModel = $sModel->where('domain','like',DB::raw("'%$v%'"));
                    $filter_arr[$k] = $v;
                    
                }elseif($k=='end_date_than'){
                    $sModel = $sModel->whereDate('end_date','>',$v);
                    $filter_arr[$k] = $v;
                    
                }elseif($k=='end_date_less'){
                    $sModel = $sModel->whereDate('end_date','<',$v);
                    $filter_arr[$k] = $v;
                    
                }else{
                    if(is_string($v)){
                        $sModel = $sModel->where([$k=>$v]);
                        $filter_arr[$k] = $v;
                    }
            
                    if(is_array($v)){
                        $sModel = $sModel->whereIn($k,$v);
                        $filter_arr[$k] = implode(',',$v);
                    }
                }
                
            }
            
            $data = $sModel->where('user_id',$this->user->id)->with('invite_user')->OrderBy('id','desc')->paginate($page_size,['*'],'page',$page_no);
            
            $filter_arr['filter'] = 'true';
            $this->setData('filter_arr',$filter_arr);
            $comma = '';
            $filter_str='';
            foreach($filter_arr as $k=>$v){
                $filter_str .= $comma.$k.'='.$v;
                $comma = '&';
            }
            if($filter_str){
                $filter_str .= '&filter=true';
                $this->setData('filter_str',$filter_str);
            }
            
        }else{
            $this->setData('filter_arr',[]);
            $objectModel = $objectModel->where('user_id',$this->user->id);
            $data = $objectModel->OrderBy('id','desc')->with('invite_user')->paginate($page_size,['*'],'page',$page_no);
            
        }
        
    }
    
}

